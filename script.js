...
function drawMinimap() {
  ...
  const VIEW_RADIUS = 12; // how many tiles around player to show
  ...
  ctx.lineTo(
    startX +
      (VIEW_RADIUS + Math.cos(player.angle) * 0.8) * MINIMAP_SCALE + (VIEW_RADIUS / 2),
    startY + 
      (VIEW_RADIUS + Math.sin(player.angle) * 0.8) * MINIMAP_SCALE + (VIEW_RADIUS / 2)
  );
  ctx.stroke();
}
...
function render() {
  ...
  for (let i = 0; i < RAYS; i++) {
    ...
    // DIFFERENT color per wall face
    ctx.fillStyle = hit.vertical ? "#ff3b3b" : "#3b7cff";
